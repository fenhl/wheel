...
impl Issue {
    pub async fn set_labels(&self, client: &Client, token: &mut github_app_auth::InstallationAccessToken, repo: &Repo, labels: &[String]) -> Result {
...
impl Repo {
    ...
    pub async fn issues_with_label(&self, client: &Client, token: &mut github_app_auth::InstallationAccessToken, label: &str) -> Result<Vec<Issue>> {
    ...
    pub async fn latest_release(&self, client: &Client) -> Result<Option<Release>> {
    ...
    pub fn latest_release_sync(&self, client: &reqwest::blocking::Client) -> Result<Option<Release>> {
        let response = client.get(&format!("https://api.github.com/repos/{}/{}/releases/latest", self.user, self.name))
            .send()?;
        if response.status() == StatusCode::NOT_FOUND { return Ok(None) } // no releases yet
        Ok(Some(
    ...
    pub async fn release_by_tag(&self, client: &Client, tag: &str) -> Result<Option<Release>> {
    ...
    pub async fn create_release(&self, client: &Client, name: String, tag_name: String, body: String) -> Result<Release> {
        Ok(
    ...
    pub async fn publish_release(&self, client: &Client, release: Release) -> Result<Release> {
        Ok(
    ...
    pub fn release_attach<'a>(&self, client: &'a Client, release: &'a Release, name: &'a str, content_type: &'static str, body: impl Into<Body> + 'a) -> impl Future<Output = Result> + 'a {
